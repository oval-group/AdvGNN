...
import torch
from lp_solver.modules import View, Flatten
from torch.nn.parameter import Parameter
from lp_solver.model import simplify_network
import random
import copy
import json
...
def load_cifar_1to1_exp(model, idx, test=None, cifar_test=None, return_true_class=False, printing=False, download=False):
    ...
    if cifar_test is None:
        ...
        cifar_test = datasets.CIFAR10('./cifardata/', train=False, download=download,
...
def max_pool(candi_tot, lb_abs, change_sign=False):
    ...
    while candi_tot > 1:
        ...
        for idl in temp:
            sum_layer_weight[idl, 2*idl] = 1
            sum_layer_weight[idl, 2*idl+1] = 1
        #######
...
def load_1to1_eth(dataset, model, idx=None, test=None, mnist_test=None, eps_temp=None, lb_abs=-1000,
                  max_solver_batch=10000, return_true_class=False):
    device = 'cpu'
    if model == 'mnist_0.1':
    ...
    if y_pred != y:
        ...
        for layer in added_prop_layers:
            for p in layer.parameters():
                p.requires_grad = False
        if return_true_class:
            return x, added_prop_layers, None, domain, y_pred, model
        else:
